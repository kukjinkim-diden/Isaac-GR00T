...
def build_diden_humanoid_hand_config() -> "dict[str, ModalityConfig]":
    ...
    return {
        "video": ModalityConfig(
            delta_indices=[0],
            modality_keys=["ego_cam", "right_wrist_cam"],
        ),
        "state": ModalityConfig(
            delta_indices=[0],
            modality_keys=["joint_pos", "joint_vel", "ee_pos", "finger_right", "fingertip_right"],
        ),
        "action": ModalityConfig(
            delta_indices=list(range(0, 16)),  # 16-step prediction horizon
            modality_keys=["joint_pos", "finger_right"],
            action_configs=[
...
def register_diden_humanoid_right_hand() -> None:
